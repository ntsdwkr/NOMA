%% NOMA vs OMA BER comparison for 2 user scenario using OFDM blocks
...
userDistances = [60, 20];     % User distances in meters
...
SNRdBRange = -10:10:60;        % SNR values in dB
...
h = cell(1, 2);                % Cell array to store channel responses
% User data and modulated signal for all user
dataUser = cell(1, 2);         % Cell array to store user data (NOMA)
signalUser = cell(1, 2);       % Cell array to store user signal (NOMA)
%% NOMA Simulation
% Arrays to store BER results for NOMA
BERUser_NOMA = zeros(2, length(SNRdBRange));  % Rows: Users, Columns: SNR values
for SNRIdx = 1:length(SNRdBRange)
    SNRdB = SNRdBRange(SNRIdx);
    SNR = 10^(SNRdB/10);
    noiseVar = meanSquare * signalPower / SNR;
    BERPerBlock = zeros(2, numBlocks); % Array to store BER per block
    for i = 1:numBlocks
        % Transmitter
        for user = 1:2
        ...
        signal = sqrt(0.75) * signalUser{1} + sqrt(0.25) * signalUser{2};
        ...
        for user = 1:2
            if user == 1
                ...
                estimatedSymbol2_1 = estimatedSymbol2 - sqrt(0.75) * (2 * estimatedBits21 - 1);
...
BERUser_OMA = zeros(2, length(SNRdBRange));  % Rows: Users, Columns: SNR values
for user = 1:2
    for SNRIdx = 1:length(SNRdBRange)
        SNRdB = SNRdBRange(SNRIdx);
        SNR = 10^(SNRdB/10);
...
legend('NOMA User 1 (weaker)', 'OMA User 1 (weaker)', 'NOMA User 2 (stronger)', ...
        'OMA User 2 (stronger)', 'Interpreter', 'Latex');
ylim([10^-5 10^-1]);
grid on;
